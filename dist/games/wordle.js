...
// ─── factory ─────────────────────────────────────────────────────────────────
export function createWordleGame(canvas, ctx) {
    ...
    function letterStateKeyColor(s) {
        switch (s) {
            case LetterState.Correct: return "#6aaa64";
            case LetterState.Present: return "#c9b458";
    ...
    function drawFlipTile(row, col, char, progress) {
        ...
        drawText(ctx, char.toUpperCase(), GRID_SIZE * col + GRID_SIZE / 2, GRID_SIZE * row + GRID_SIZE / 2 + 2, "#000", `${Math.round(FONT_SIZE)}px Arial`, "center", "middle");
        ctx.restore();
    }
    ...
    function colorsForGuess(word) {
        ...
        const result = new Array(MAX_COLS).fill(LetterState.Absent);
    ...
    function updateKeyboardColors() {
        ...
        for (let i = 0; i < guesses.length; i++) {
            ...
            for (let col = 0; col < letters.length; col++) {
                ...
                if (existing === letterStateColor(LetterState.Correct))
                    continue;
    ...
    async function start() {
        ...
        _onKeyDown = (e) => handleKeyInput(e.key, "keyboard");
        _onResize = updateKeyboardDimensions;
        _onOrientationChange = updateKeyboardDimensions;
        _onMouseUp = (e) => handleCanvasClick(e.clientX, e.clientY, "mouse");
        _onTouchStart = (e) => {
            e.preventDefault();

...
export function createWeaverGame(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D): Game {
	...
	let startWord: string = "WIND";
	let targetWord: string = "SAIL";
	let guesses: string[] = [];
	let gameState: GameState = GameState.Playing;
	let validWords = new Set<string>();
	let wordList: string[] = [];
	let wildcardBuckets = new Map<string, string[]>();
	let optimalMoves = 0;
	...
	function buildWildcardBuckets(words: string[]): Map<string, string[]> {
	...
	function getShortestMoveCount(start: string, target: string): number {
	...
	function getShortestPath(start: string, target: string): string[] {
	...
	function getPuzzleMetrics(start: string, target: string): { valid: boolean; optimalMoves: number; optimalWordsBetween: number } {
		if (start === target) {
	...
	function pickRandomPuzzlePair(words: string[]): { start: string; target: string; optimalMoves: number; optimalWordsBetween: number } {
		if (words.length < 2) {
	...
	function differsByOneLetter(candidate: string, previous: string): boolean {
	...
	function getVisibleGuesses(): string[] {
		if (guesses.length <= MAX_VISIBLE_GUESSES) return guesses;
		return guesses.slice(guesses.length - MAX_VISIBLE_GUESSES);
	}

	function getCollapsedGuessCount(): number {
		return Math.max(0, guesses.length - MAX_VISIBLE_GUESSES);
	}

	function getBoardRows(): number {
	...
	function drawRoundRect(row: number, col: number, color = "#EEE"): void {
	...
	function drawCharacter(char: string, row: number, col: number, color = "#000"): void {
	...
	function isCorrectColumnLetter(letter: string | undefined, col: number): boolean {
		if (!letter) return false;
		return letter.toUpperCase() === (targetWord[col] ?? "").toUpperCase();
	}

	function drawWord(word: string, row: number, checkAgainstTarget = false): void {
	...
	function wasTargetLetterGuessed(letter: string, col: number): boolean {
		if (!letter) return false;
		for (const guess of guesses) {
			if ((guess[col] ?? "").toUpperCase() === letter.toUpperCase()) return true;
		}
		return false;
	}

	function drawTargetWord(targetWord: string, row: number): void {
	...
	function drawCollapsedGuessesRow(row: number, hiddenCount: number): void {
		for (let col = 0; col < BOARD_COLS; col++) {
		...
		drawText(
			...
			label,
			CANVAS_WIDTH / 2,
			BOARD_TOP + GRID_SIZE * row + GRID_SIZE / 2 + 1,
			"#666",
			`${Math.round(FONT_SIZE * 0.32)}px Arial`,
			"center",
			"middle"
		);
	}
	...
	function makeGuess(word: string): void {
	...
	function handleTemplateInput(key: string): void {
	...
	async function start(): Promise<void> {
		...
		_onKeyDown = (e: KeyboardEvent) => handleTemplateInput(e.key);
		_onResize = updateDimensions;
		_onOrientationChange = updateDimensions;
		_onMouseUp = (e: MouseEvent) => handleCanvasClick(e.clientX, e.clientY);
